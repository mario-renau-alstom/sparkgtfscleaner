...
class Torrejon  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, spark: SparkSession) = {
   ...
   println("Torrejon GTFS Processing")
   println("URL: " + urlfile)
   println("-----------------------------------------------")
   println("")
   ...
   println("")
   IOOperations.ExtractFiles(urlfile, "torrejon_gtfs", workPath, backupPath, sourcesPath, spark)

   var dataframes = FixOperations(sourcesPath, "torrejon_gtfs", spark)
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    val raw_path = rawPath.concat("GTFSCLEAN/" + "torrejon_gtfs/")
