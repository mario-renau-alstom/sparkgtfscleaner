...
import com.alstom.GTFSOperations.IOOperations.getFileNameAndExtFromPath
...
class MadridAutobusUrbano  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, spark: SparkSession) = {
   ...
   println("")
   IOOperations.ExtractFiles(urlfile, "madrid_autobus_urbano_gtfs", workPath, backupPath, sourcesPath, spark)

   var dataframes = FixOperations(sourcesPath, "madrid_autobus_urbano_gtfs", spark)
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    val raw_path = rawPath.concat("GTFSCLEAN/" + "madrid_autobus_urbano_gtfs/")
