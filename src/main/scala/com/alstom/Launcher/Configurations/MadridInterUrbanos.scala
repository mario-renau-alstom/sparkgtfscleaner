...
class MadridInterUrbanos  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, spark: SparkSession) = {
   ...
   println("")
   IOOperations.ExtractFiles(urlfile, "madrid_interurbanos_gtfs", workPath, backupPath, sourcesPath, spark)

   var dataframes = FixOperations(sourcesPath, "madrid_interurbanos_gtfs", spark)
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    val raw_path = rawPath.concat("GTFSCLEAN/" + "madrid_interurbanos_gtfs/")
