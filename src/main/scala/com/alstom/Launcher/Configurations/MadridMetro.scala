...
class MadridMetro  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, spark: SparkSession) = {
   ...
   println("Madrid Metro GTFS Processing")
   println("URL: " + urlfile)
   println("-----------------------------------------------")
   println("")
   ...
   println("")
   IOOperations.ExtractFiles(urlfile, "madrid_metro_gtfs", workPath, backupPath, sourcesPath, spark)

   var dataframes = FixOperations(sourcesPath, "madrid_metro_gtfs", spark)
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    val raw_path = rawPath.concat("GTFSCLEAN/" + "madrid_metro_gtfs/")
