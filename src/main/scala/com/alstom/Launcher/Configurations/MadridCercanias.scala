...
class MadridCercanias  {
 ...
 def Process(workPath: String, backupPath: String, sourcesPath: String, rawPath: String, urlfile: String, spark: SparkSession) = {
   ...
   println("")
   IOOperations.ExtractFiles(urlfile, "madrid_cercanias_gtfs", workPath, backupPath, sourcesPath, spark)

   var dataframes = FixOperations(sourcesPath,"madrid_cercanias_gtfs", spark)
  ...
  def UploadAzure(dataframes: List[DataFrame], rawPath: String, spark: SparkSession) = {
    ...
    val raw_path = rawPath.concat("GTFSCLEAN/" + "madrid_cercanias_gtfs/")
